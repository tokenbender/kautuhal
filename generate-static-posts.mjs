    ...
    return `(function () {
        ...
        const updateShader = \`
            ...
            fn main(@builtin(global_invocation_id) gid: vec3<u32>) {
                ...
                for (var c: u32 = 0u; c < CHANNELS; c = c + 1u) {
                    ...
                    proposal[index] = state_in[index] + delta;
                }
            }
        \`;
        ...
        const renderShader = \`
            ...
            fn fragment_main(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
                ...
                let light_gold = vec3<f32>(139.0, 94.0, 0.0) / 255.0;
                let color = select(light_gold, dark_gold, params.z > 0.5);
        ...
        const updateUniforms = function () {
            ...
            device.queue.writeBuffer(renderParamsBuffer, 0, new Float32Array([
                canvas.width, canvas.height, isDark() ? 1.0 : 0.0, isDark() ? 0.48 : 0.34
            ]));
        };
        ...
        const loop = function () {
            ...
            const startedAt = performance.now();
            step();
            if (reducedMotion && steps >= 64) return;
            const interval = steps < 64 ? 65 : 100;
        ...
        const initialize = async function () {
            ...
            resizeCanvas();
            label.innerHTML = '<strong>Neural Cellular Automaton / 2,940 Parameters</strong>Hover Anywhere to Damage · ~5s Recovery';
